...
extension BDChatKFViewController {
    ...
    func sendTextMessage(_ content: String?) {
        ...
        let messageModel = self.insertLocalMessageAndReload(content: content!, type: BytedeskConstants.MESSAGE_TYPE_TEXT)
//        BDCoreApis.sendTextMessageProtobuf(mid: localId, content: content!, thread: mThreadModel!)
        
        let encoder = JSONEncoder()
        let messageJson = try! encoder.encode(messageModel)
        let messageString: String? = String(data: messageJson, encoding: .utf8)
        debugPrint("sendTextMessage messageJson: \(messageString!)")
        
        if (BytedeskStompApi.sharedInstance().isConnected()) {
            BytedeskStompApi.sharedInstance().sendMessage(message: messageString!)
        } else {

            BytedeskHttpApis.sharedInstance().sendRestMessage(json: messageString!) { messageResult in
